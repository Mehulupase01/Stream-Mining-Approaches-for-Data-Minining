...
def reservoir_sampling(k, datastream):
    ...
    """
    # Initialize an empty list for the sample
    
    sample = []
    # Iterate over the datastream, using enumerate to get the index (0-based) and transaction
    for index, transaction in enumerate(datastream()):
        if index < k:
            #Filling the initial sample list until it reaches size k
            sample.append(transaction)
        else:
            # Generate a random index between 0 and index, we select a random integer between 0 and index.
            rand_index = random.randint(0, index)
            # Now if the random index is less than k, we'll just replace an element in the sample.
            if rand_index < k:
                sample[rand_index] = transaction
